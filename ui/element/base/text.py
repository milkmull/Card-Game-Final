...
class Text:
    ...
    def style_text(text):
        ...
        for _match in re.finditer(r"(?<![a-zA-Z0-9^_])([0-9]+)", text):
            style.update({i: number_style for i in range(_match.start(), _match.end())})
            
        keyword_style = {"fgcolor": (147, 199, 99), "style": 1}
        for word in keyword.kwlist:
            for _match in re.finditer(fr"(?<![a-zA-Z0-9^_])({word})(?![a-zA-Z0-9^_])", text):
                style.update({i: keyword_style for i in range(_match.start(), _match.end())})
       
        string_style = {"fgcolor": (236, 118, 0)}
        for _match in re.finditer(r"(['\"])(.*?)\1", text):
            style.update({i: string_style for i in range(_match.start(), _match.end())})
            
        class_style = {"fgcolor": (160, 130, 189), "style": 1}
        for _match in re.finditer(r"(?<=class )([a-zA-Z0-9_]+)", text):
            style.update({i: class_style for i in range(_match.start(), _match.end())})
            
        def_style = {"fgcolor": (103, 140, 177), "style": 1}
        for _match in re.finditer(r"(?<=def )([a-zA-Z0-9_]+)", text):
            style.update({i: def_style for i in range(_match.start(), _match.end())})
